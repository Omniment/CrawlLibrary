/**
 * @file crawl.h
 * @brief クロールに対して指令を出す,クロールの持っているセンサの情報を取得するためのライブラリ.
 */
...
class CrlRobot {
 public:
   ...
   *
   * @return なし
   * @warning このメンバ関数を呼ばずに他のメンバ関数を呼び出すと予期せぬエラーが発生します.
   * @attention キャリブレーション中に水平な場所に置き,安静にしない場合,正しくセンサの値を得られません.
   */
  void init();
  /**
   ...
   *
   * @return なし
   * @warning ループ内の処理がdt秒を超える場合にはこのメンバ関数は正しく動作せず,直ちにリターンします.
   * @sa set_dt(float dt)
   */
  void realtimeLoop();
  /**
   ...
   * @param dt ループ間隔 単位:秒
   * @return なし
   * @warning このメンバ関数を呼び出さない場合ループ間隔として,初期値として0秒が指定され,
   ...
   * @return なし
   * @attention このメンバ関数が呼ばれない限り,モータ出力や,センサの値は更新されません.
   * @note 制御ループを実装する場合にはrealtimeLoop()の直下で呼び出すことを推奨します.
   */
  void updateState();
